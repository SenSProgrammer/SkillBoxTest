...
const addNewTimerForUserId = async (userId, name) => {
  ...
  return knex("timers").insert(TM);
};

const getActiveTimersByUserId = async (userId) => knex("timers").select().where({ user_id: userId, is_active: true });

const getStoppedTimersByUserId = async (userId) => knex("timers").select().where({ user_id: userId, is_active: false });
