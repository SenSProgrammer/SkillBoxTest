...
import { getTree, getThesis, getComments, postComment, login } from "./api";
...
export default function App() {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "240px 1fr 360px 360px", gap: 16, padding: 16 }}>
      <aside><h3>Тезисы</h3><Tree nodes={tree} onSelect={loadThesis} /></aside>

      <main>
        <h3>{thesis?.slug || "Выберите тезис"}</h3>
        <ThesisView html={thesis?.textHtml} />
      </main>
      ...
      <aside>
        <h3>Вход</h3>
        {user ? (
          <div>В системе: {user.email} ({user.role})</div>
        ) : (
